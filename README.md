...
Чистый результат на seed 42 (мок-среда): **3 170 330 у.е.** — код коммита `701015c` (после заморозки `f11ae2d`, тот же алгоритм), см. [docs/PLAN_EXAMPLE.md](docs/PLAN_EXAMPLE.md).
...
|---|---|---|
| Чистый результат, seed 42 (`python local_eval.py`) | 3 170 330 у.е. (валовой прирост 3 270 248, затраты 99 918, ROI 32.73×) | [docs/PLAN_EXAMPLE.md](docs/PLAN_EXAMPLE.md), код коммита `701015c` — после заморозки `f11ae2d`, тот же алгоритм |
| Медиана по 10 прогонам (`python local_eval.py --runs 10`) | 568 215 у.е. (минимум 72 682) | [docs/BENCHMARK.md](docs/BENCHMARK.md), измерено на `b8b42ec` — **до policy v4**, не переизмерено на замороженном коде |
| Устойчивость на стресс-тесте (6 вариантов × seeds 0–9, `tools/stress_eval.py`) | 50 из 60 прогонов в плюс (по вариантам: baseline 10/10, ×0.5 arpu 6/10, ×1.5 arpu 10/10, случайное ×U(0.5,1.5) 10/10, смена знака 20% 7/10, conversion×0.5 7/10) | [docs/ROBUSTNESS.md](docs/ROBUSTNESS.md), измерено на `98c0980` — **до policy v4**, не переизмерено на замороженном коде |
| По сравнению с naive-стратегией (baseline-вариант) | naive: медиана 1 121 837 у.е. — выше нашего результата (568 215) на том же (старом) прогоне | [docs/BENCHMARK.md](docs/BENCHMARK.md), `b8b42ec` — **до policy v4** |
| По сравнению с template — базовым агентом организаторов (baseline-вариант) | template: медиана −357 948 у.е. (отрицательно) против наших 568 215 на том же (старом) прогоне | [docs/BENCHMARK.md](docs/BENCHMARK.md), `b8b42ec` — **до policy v4** |
| Доля от oracle-плана с полной информацией (baseline-вариант) | 11,4% от медианы oracle-плана | [docs/BENCHMARK.md](docs/BENCHMARK.md), `b8b42ec` — **до policy v4** |

Свежий прогон после заморозки есть только для одного примера плана ([docs/PLAN_EXAMPLE.md](docs/PLAN_EXAMPLE.md), seed 42); агрегированные метрики устойчивости и сравнения выше измерены на более старой версии агента (до policy v4) и пока не переизмерены на замороженном коде.
...
3. Ожидаемый результат — блок с `Статус: PASS` или `FAIL`, чистым
   ...
   [docs/PLAN_EXAMPLE.md](docs/PLAN_EXAMPLE.md), код коммита `701015c`
   (после заморозки `f11ae2d`, тот же алгоритм):
   ```
   Статус: PASS
...
Таблица ниже — измерение на коммите `98c0980`, **до policy v4**, из [docs/ROBUSTNESS.md](docs/ROBUSTNESS.md); после заморозки `f11ae2d` эти метрики не переизмерены.
...
- 10 из 10 прогонов `local_eval.py --runs 10` в плюс на baseline
  ([docs/BENCHMARK.md](docs/BENCHMARK.md), коммит `b8b42ec`, **до policy v4**;
  не переизмерено на замороженном коде) — это про устойчивость на
