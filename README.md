...
В истории изменений `agent.py` и `agent_core/` (`git log
--format='%h %s%n%b' -- agent.py agent_core/`) есть следующие трейлеры
`Co-Authored-By`; авторы указаны по полю Author коммитов:

| Коммиты | Автор | `Co-Authored-By` |
|---|---|---|
| `7dd08a6`, `aca319c`, `d9b2b8e`, `35957a3` | Beka Raissov | Claude Opus 5.5 (1M context) &lt;noreply@anthropic.com&gt; |
| `ee1ce29` | razhaksyg | Claude Opus 5.5 &lt;noreply@anthropic.com&gt; |

Это зафиксированные в Git сведения о соавторстве. Отсутствие такого трейлера
в других коммитах не доказывает ни использование, ни неиспользование
AI-инструментов; конкретный клиент (например, Codex CLI) эти трейлеры не указывают.
...
Для опционального советника нужен ключ API (см. раздел 7).
