...
were measured on `main` at `b8b42ec` (2026-09-23), seeds 0–9, with the LLM
...
| Baseline | Ours | 568,215 | 72,682 | 10/10 | 11.4% |
...
| `arpu_change_pct` ×0.5 | Ours | 18,626 | -46,993 | 7/10 | 0.7% |
...
| `arpu_change_pct` ×1.5 | Ours | 938,762 | 152,020 | 10/10 | 12.5% |
...
| Per-row ×U(0.5, 1.5) | Ours | 557,283 | 30,905 | 10/10 | 11.0% |
...
| Flip 20% of positive rows | Ours | 59,847 | -165,386 | 7/10 | 1.4% |
...
| `conversion_rate` ×0.5 | Ours | 30,479 | 5,990 | 10/10 | 1.2% |
...
- **Ours:** the unchanged `agent.py` at the tested commit.
...
| --- | ---: | ---: |
| Baseline | 9,015,764 | 6.3% |
| `arpu_change_pct` ×0.5 | 4,269,339 | 0.4% |
| `arpu_change_pct` ×1.5 | 13,971,534 | 6.7% |
| Per-row ×U(0.5, 1.5) | 9,278,223 | 6.0% |
| Flip 20% of positive rows | 7,629,715 | 0.8% |
| `conversion_rate` ×0.5 | 3,999,579 | 0.8% |

The naive policy beats ours on this mock in every variant's median, although
ours avoids the template's consistently negative runs. Ours still has three
negative seeds in each of the half-effect and sign-flip variants. These numbers
measure the local mock and its defined perturbations, not hidden judge effects.
