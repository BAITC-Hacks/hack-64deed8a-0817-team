...
> Исторический отчёт: снят на `aca319c` (agent v2), находка §2 закрыта в `e070ffd`.
> Числа ниже относятся к тем коммитам. Актуальные числа замороженного кода `f11ae2d`
> (policy v4: seed 42 — 3 170 330, медиана 10 прогонов 3 541 278, stress 60/60) — в
> README.md, docs/BENCHMARK.md и docs/ROBUSTNESS.md.
...
## Итог: изначально 1 находка требовала решения капитана — решена (см. §2), всё чисто

| # | Проверка | Статус |
|---|---|---|
| 1 | Запрещённые паттерны (MECHANICS.md §7) | ✅ чисто |
| 2 | Воспроизводимость с fresh clone | ✅ **решено в коммите [e070ffd](../commit/e070ffd)** |
...
**Обновление 23.09.2026:** находка решена коммитом [e070ffd](../commit/e070ffd)
(`LLM advisor with guardrails (optional, OPENAI_API_KEY)`, автор Beka Raissov),
который обновил `submission.csv` вместе с изменениями `agent_core`. 23.09.2026
повторная проверка показала, что `python make_submission.py` больше не создаёт
diff с закоммиченным файлом.
...
| 5. `submission.csv` воспроизводится | Файл соответствует выводу `make_submission.py`; обновлено в [e070ffd](../commit/e070ffd) | ✅ |
...
2. §8 «Пример реального вывода»: `net 60,265`, было снято на `agent.py` v1
   (коммит `6b871bb`). `agent.py` v2 (`aca319c`, на момент аудита) на том же seed 42
   давал `net 676,992` — **исправлено**, пример обновлён с пометкой версии
   агента.
3. §9 «Ограничения»: «≈3 из 10 прогонов в плюс» — было верно для v1, на v2
   (`aca319c`) **10 из 10 в плюс** (медиана ≈575k, минимум ≈60k) — **исправлено**,
