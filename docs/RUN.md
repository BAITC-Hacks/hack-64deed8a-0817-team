...
`local_eval.py --runs 10` на текущем `agent.py` даёт разный знак чистого результата
между seed (см. вывод команды выше) — механика (лимиты/бюджет/дедуп) отрабатывает
идентично судейству, но сами эффекты в мок-среде — заглушка (docs/MECHANICS.md,
